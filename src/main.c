...
int main() {
    ...
    stdio_init_all();
    printf("[MAIN] stdio initialized.\n");

    // Seed RNG
    srand(time_us_32());
    printf("[MAIN] RNG seeded.\n");
    
    // Initialize I2C
    printf("[MAIN] Initializing I2C...\n");
    init_i2c();
    sleep_ms(500);
    printf("[MAIN] I2C initialized.\n");
    
    // Initialize NeoPixels
    printf("[MAIN] Initializing NeoPixels...\n");
    if (init_neopixels() < 0) {
        printf("[MAIN] ERROR: Failed to initialize NeoPixels.\n");
        return 1;
    }
    printf("[MAIN] NeoPixels initialized.\n");

    // Initialize the game (sets up keypad callback, timers, first target)
    game_init();
    printf("[MAIN] Game initialized. Entering game loop...\n");

    while (true) {
        game_step();   // all game logic lives in game.c now
    }
